...
(* ==================================================================== *)
(*  Dolomite in NaCl -- aqueous speciation from published constants     *)
(*  (fixed pCO2, pH calculated from the mass balance)                   *)
(*                                                                      *)
(*  Published TOUGHREACT / EQ3-6 constants; compared with Visual MINTEQ.*)
(*   - Carbonate is OPEN: {CO2(aq)} is fixed (fixed pCO2).              *)
(*   - pH is CALCULATED from the proton mass balance, with              *)
...
(* -------------------------------------------------------------------- *)
(*  log K are published TOUGHREACT / EQ3-6 values                        *)
(*  (Plummer & Busenberg 1982; Nordstrom et al. 1990; Wolery 1992).      *)
(*  Water / carbonate                                                    *)
(*    H2O            = H+ + OH-                    logK = -13.995         *)
(*    CO2(aq) + H2O  = H+ + HCO3-                  logK =  -6.345         *)
(*    HCO3-          = H+ + CO3^2-                 logK = -10.329         *)
(*    CO2(g)         = CO2(aq)  -> {CO2(aq)} fixed by pCO2 = 10^-3.5      *)
(*  Complexes                                                            *)
...
(* -------------------------------------------------------------------- *)
(* published TOUGHREACT / EQ3-6 constants; nothing taken from MINTEQ      *)
...
aCO2 = 10^-1.469 * 10^-3.5;   (* {CO2(aq)} fixed by atmospheric pCO2=10^-3.5, Henry logKH=-1.469 *)
ADavies = 0.509;
daviesB = 0.5;            (* Davies salting term *)
...
(*      log g_z = -A z^2 ( sqrt I/(1+sqrt I) - b I ), b=0.5 ;  g0 = 1    *)
(* -------------------------------------------------------------------- *)
gammas[ii_?NumericQ] := If[ii <= 0, {1.,1.,1.},
  Module[{f = -ADavies (Sqrt[ii]/(1+Sqrt[ii]) - daviesB ii)}, {10^f, 10^(4 f), 1.}]];
